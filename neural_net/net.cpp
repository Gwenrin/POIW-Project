...
vector<vector<vector<float>>> ImageToMap(const vector<vector<int>>& Image) {
    ...
    size_t paddedSize = imageDimension + 4; // 2 pixels of padding on each side
    vector<vector<vector<float>>> Input(paddedSize, vector<vector<float>>(paddedSize, vector<float>(1, 0.0f)));

    for (size_t i = 0; i < imageDimension; i++) {
        for (size_t j = 0; j < imageDimension; j++) {
            Input[i + 2][j + 2][0] = (float)Image[i][j]; // offset by 2
        }
    }
    return Input;
}
...
char ForwardPass(const vector<vector<int>>& Image, Network& net, ForwardPassData& fwdPass) {
